#!/usr/bin/env ts-node
/**
 * Diagnostic script to show detailed ladder test failures
 */

import { appendDoc, loadGraph, clearStorage } from './app/storage/storage';
import * as path from 'path';

// Copy the test cases from level-1-simple.spec.ts
...
async function main() {
  const testPath = path.join(process.cwd(), 'diagnose-ladder-1.json');

  console.log('🔍 DIAGNOSING LEVEL 1 FAILURES\n');
  ...
  for (const tc of testCases) {
    ...
    if (!graph) {
      console.log(`❌ Test ${tc.id} - No graph extracted`);
      continue;
    }
    ...
    if (entityP < 0.90 || entityR < 0.85 || relationP < 0.90 || relationR < 0.85) {
      ...
      if (extraEntities.length > 0) {
        console.log(`   Extra entities: ${extraEntities.join(', ')}`);
      }

      // Show relation differences
      ...
      if (extraRelations.length > 0) {
        console.log(`   Extra relations: ${extraRelations.join(', ')}`);
      }
      console.log();
    } else {
  ...
  console.log(`\n❌ Failed: ${failCount}/${testCases.length} tests\n`);
