...
const dependency_paths_1 = require("./relations/dependency-paths");
...
function traceRelation(stage, rel) {
    if (!TRACE_REL)
        return;
    try {
        fs.appendFileSync("tmp/relation-trace.log", JSON.stringify({
            stage,
            pred: rel.pred,
...
function normalizeAliasSurface(s) {
    if (!s)
        return s;
    let x = s.trim();
...
function pickBestEntityId(surface, candidateIds, entities) {
...
function getEntityCandidatesByOffset(spans, start, end) {
    if (!spans.length)
        return [];
...
function selectEntitySpan(entityId, spans, preferRange) {
    const entitySpans = spans.filter(sp => sp.entity_id === entityId);
    if (!entitySpans.length)
        return null;
    if (!preferRange)
        return entitySpans[0];
    const overlapping = entitySpans.find(sp => rangesOverlap(sp.start, sp.end, preferRange.start, preferRange.end));
    if (overlapping)
        return overlapping;
...
function mapSurfaceToEntity(surface, entities, spans, preferRange) {
    const normSurface = normalizeAliasSurface(surface);
    if (!normSurface)
        return null;
    ...
    if (!candidateIds.length)
        return null;
    const bestId = pickBestEntityId(surface, candidateIds, entities) ??
...
function collectConjTokens(base, tokens) {
    const result = [];
    const queue = [base];
    const seen = new Set();
    while (queue.length) {
        const current = queue.pop();
        if (seen.has(current.i))
            continue;
        seen.add(current.i);
        result.push(current);
...
function findPrepObject(root, tokens, preps) {
    const queue = [root];
    const seen = new Set();
    while (queue.length) {
...
function buildOrgSpan(token, tokens) {
...
function resolveSubjectToken(verb, tokens) {
 ...
 */
function extractQualifiers(tokens, triggerIdx, entities, spans, text) {
    const qualifiers = [];
    ...
    for (const entity of entities) {
        if (entity.type !== 'DATE' && entity.type !== 'PLACE')
            continue;
        // Find entity span that overlaps with window
        const entitySpan = spans.find(s => s.entity_id === entity.id &&
            s.start < winEnd &&
            s.end > winStart);
        if (!entitySpan)
            continue;
 ...
 */
function tryCreateRelation(text, entities, spans, pred, subjStart, subjEnd, objStart, objEnd, source, tokens, triggerIdx) {
    ...
    let subjSpanOverride = null;
    let objSpanOverride = null;
    if (tokens && triggerIdx !== undefined && subjRef) {
        const triggerToken = tokens.find(tok => tok.i === triggerIdx);
        const voiceInfo = (0, voice_detector_1.detectVoice)(triggerToken, tokens);
    ...
    const expandCoordinationRefs = (ref, baseSpan) => {
        ...
        const segments = [];
        let current = [];
        const isCoordToken = (tok) => isCoordDelimiter(tok);
        ...
        for (const segment of segments) {
            ...
            const mapped = mapSurfaceToEntity(text.slice(segRange.start, segRange.end), entities, spans, segRange);
    ...
    const extendSpanForCoordination = (baseSpan) => {
        ...
        const members = [...baseTokens];
        const queue = [...baseTokens];
    ...
    const objCoordinationSpan = extendSpanForCoordination({
        entity_id: objEnt.id,
    ...
    // Type guard check
    if (!(0, schema_1.passesGuard)(pred, subjEnt, objEnt)) {
        return [];
    }
    const relations = [];
 ...
 */
function extractDepRelations(text, entities, spans, sentences) {
    const relations = [];
    const textLower = text.toLowerCase();
    const spanLookup = new Map();
    for (const span of spans) {
    ...
    let lastNamedOrg = null;
    let lastSchoolOrg = null;
    const recentPersons = [];
    const handledChildrenSentences = new Set();
    const handledMemberSentences = new Set();
    const handledColonPositions = new Set();
    const emittedEnumerationKeys = new Set();
    const pushRecentPerson = (tok) => {
        if (!tok)
            return;
        if (!isNameToken(tok))
            return;
        if (tok.pos !== 'PROPN')
            return;
        if (tok.ent && tok.ent !== 'PERSON' && tok.ent !== 'NORP')
            return;
    ...
    const pushRecentOrg = (tok) => {
        if (!tok)
            return;
        if (tok.pos !== 'PROPN')
            return;
        if (tok.ent && tok.ent !== 'ORG' && tok.ent !== 'FAC')
            return;
        lastNamedOrg = tok;
        const lower = tok.text.toLowerCase();
    ...
    const relationKey = (rel) => {
        ...
        const subjKey = subjEntity
            ? `${subjEntity.type}:${(0, entities_1.normalizeName)(subjEntity.canonical).toLowerCase()}`
            : rel.subj;
        const objKey = objEntity
            ? `${objEntity.type}:${(0, entities_1.normalizeName)(objEntity.canonical).toLowerCase()}`
            : rel.obj;
        return `${subjKey}::${rel.pred}::${objKey}`;
    };
    const getEntityById = (id) => entities.find(e => e.id === id);
    const entityFromSpan = (start, end) => {
        const surface = text.slice(start, end);
        const mapped = mapSurfaceToEntity(surface, entities, spans, { start, end });
        return mapped?.entity;
    };
    const ensureSpanForEntity = (entity, fallback) => {
        const existing = spanLookup.get(entity.id);
        if (existing)
            return existing;
        return { entity_id: entity.id, start: fallback.start, end: fallback.end };
    };
    const findNearestEntityBefore = (offset, allowedTypes, maxDistance = 240) => {
        let best = null;
    ...
    const propagateLivesIn = (rel, triggerTok) => {
        const subjEntity = getEntityById(rel.subj);
        const objEntity = getEntityById(rel.obj);
        ...
        for (const entity of entities) {
            if (entity.type !== 'PERSON' || entity.id === subjEntity.id)
                continue;
            const normalized = (0, entities_1.normalizeName)(entity.canonical).toLowerCase();
        ...
        if (!candidateEntities.length && recentPersons.length) {
            for (const token of recentPersons) {
                if (token.start >= placeSpan.start && token.start <= placeSpan.end)
                    continue;
                ...
                if (!entity || entity.type !== 'PERSON' || entity.id === subjEntity.id)
                    continue;
        ...
        for (const candidate of candidateEntities) {
            ...
            if (!personSpan)
                continue;
            if (!(0, schema_1.passesGuard)('lives_in', candidate, objEntity))
                continue;
            ...
            const inversePred = schema_1.INVERSE['lives_in'];
            if (inversePred) {
                const inverseRel = {
                    id: (0, uuid_1.v4)(),
                    subj: rel.obj,
    ...
    const addProducedRelations = (produced, triggerTok) => {
    ...
    const emitCandidate = (pred, subjRef, objRef, triggerTok) => {
        if (!subjRef || !objRef)
            return;
        ...
        const subjKey = `${subjRef.entity.type}:${(0, entities_1.normalizeName)(subjRef.entity.canonical).toLowerCase()}`;
        const objKey = `${objRef.entity.type}:${(0, entities_1.normalizeName)(objRef.entity.canonical).toLowerCase()}`;
        const enumerationKey = `${pred}::${subjKey}::${objKey}`;
        if (emittedEnumerationKeys.has(enumerationKey))
            return;
        const produced = tryCreateRelation(text, entities, spans, pred, subjSpan.start, subjSpan.end, objSpan.start, objSpan.end, 'REGEX');
        if (!produced.length)
            return;
        for (const rel of produced) {
            if (rel.subj === subjRef.entity.id && rel.obj === objRef.entity.id) {
                ...
                else if (schema_1.INVERSE[pred] === rel.pred) {
    ...
    const locateSurface = (surface, searchFrom) => {
        ...
        const normalized = (0, entities_1.normalizeName)(raw);
    ...
    for (let sentenceIdx = 0; sentenceIdx < sentences.length; sentenceIdx++) {
        ...
        const entitySpanTokens = new Map();
        for (const span of spans) {
            const entity = getEntityById(span.entity_id);
            if (!entity)
                continue;
            ...
            for (let ej = ei + 1; ej < entityIds.length; ej++) {
                ...
                for (const t1 of tokens1) {
                    for (const t2 of tokens2) {
                        const result = (0, dependency_paths_1.extractRelationFromPath)(t1, t2, tokens);
                        if (result) {
                            pathResult = result;
                            token1 = t1;
                            token2 = t2;
                ...
                if (!(0, schema_1.passesGuard)(pathResult.predicate, subjectEntity, objectEntity)) {
                    if (DEBUG_DEP) {
                ...
                const produced = tryCreateRelation(text, entities, spans, pathResult.predicate, subjSpan.start, subjSpan.end, objSpan.start, objSpan.end, 'DEP', tokens, token1.i // Use first token as trigger
                );
                if (produced.length > 0) {
                    ...
                    for (const coordToken of coordSubjects) {
                        ...
                        if (coordEntityId) {
                            ...
                            if (coordEntity && coordSpan && (0, schema_1.passesGuard)(pathResult.predicate, coordEntity, objEntity)) {
                                const coordProduced = tryCreateRelation(text, entities, spans, pathResult.predicate, coordSpan.start, coordSpan.end, objSpan.start, objSpan.end, 'DEP', tokens, coordToken.i);
                    ...
                    for (const coordToken of coordObjects) {
                        ...
                        if (coordEntityId) {
                            const coordEntity = getEntityById(coordEntityId);
                            const coordSpan = spanLookup.get(coordEntityId);
                            if (coordEntity && coordSpan && (0, schema_1.passesGuard)(pathResult.predicate, subjEntity, coordEntity)) {
                                const coordProduced = tryCreateRelation(text, entities, spans, pathResult.predicate, subjSpan.start, subjSpan.end, coordSpan.start, coordSpan.end, 'DEP', tokens, coordToken.i);
                                for (const rel of coordProduced) {
                                    // Keep same confidence as primary relation - coordinated entities deserve full confidence
                                    rel.confidence = pathResult.confidence;
                                }
        ...
        if (subjectRefs.length && partnerRefs.length) {
                ...
                if (ofPrep) {
                    ...
                    if (copula) {
                        ...
                        if (headToken) {
                            if (headToken.pos === 'PROPN' || /^[A-Z]/.test(headToken.text)) {
                                ...
                                childToken = tokens.find(t => t.dep === 'appos' &&
                                    t.head === headToken.i &&
                    ...
                    if (parentCandidates.length && childToken) {
                        ...
                        for (const parentCandidate of parentCandidates) {
                            ...
                            if (childSpan.start === parentSpan.start && childSpan.end === parentSpan.end)
                                continue;
                            const produced = tryCreateRelation(text, entities, spans, 'child_of', childSpan.start, childSpan.end, parentSpan.start, parentSpan.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
            ...
            if (parentRoleNouns.has(lemma)) {
                ...
                if (childSpan && parentCandidates.length) {
                    for (const parentCandidate of parentCandidates) {
                        const parentSpan = expandNP(parentCandidate, tokens);
                        if (parentSpan.start === childSpan.start && parentSpan.end === childSpan.end)
                            continue;
            ...
            if (lemma === 'marry' || textLower === 'married') {
                ...
                if (tok.pos === 'VERB' || tok.dep === 'ROOT') {
                    ...
                    if (subj && !obj) {
                        ...
                        if (conjSubj) {
                            ...
                            const produced = tryCreateRelation(text, entities, spans, 'married_to', subj.start, subj.end, conjSubj.start, conjSubj.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
                ...
                if (subj && obj) {
                    ...
                    updateLastNamedSubject(subj);
                    const produced = tryCreateRelation(text, entities, spans, 'married_to', subj.start, subj.end, obj.start, obj.end, 'DEP', tokens, tok.i);
                    addProducedRelations(produced, tok);
                }
            }
    ...
    const workVerbs = ['work', 'works', 'worked', 'working', 'employ', 'employed', 'join', 'joined', 'joins', 'joining', 'hire', 'hired', 'recruit', 'recruited'];
        ...
        let subjTok = resolveSubjectToken(tok, tokens);
                ...
                if (subjTok && atPrep) {
                    ...
                    if (orgTok) {
                        const subjSpan = expandNP(subjTok, tokens);
                        const orgSpan = expandNP(orgTok, tokens);
                        const produced = tryCreateRelation(text, entities, spans, 'member_of', subjSpan.start, subjSpan.end, orgSpan.start, orgSpan.end, 'DEP', tokens, tok.i);
                        addProducedRelations(produced, tok);
                    }
                }
                ...
                if (lemma === 'hire' || lemma === 'recruit' || textLower === 'hired' || textLower === 'recruited') {
                    ...
                    if (objTok && subjTok) {
                        ...
                        const produced = tryCreateRelation(text, entities, spans, 'member_of', personSpan.start, personSpan.end, orgSpan.start, orgSpan.end, 'DEP', tokens, tok.i);
                        addProducedRelations(produced, tok);
                    }
                }
            }
            ...
            if (foundVerbs.includes(lemma) || foundVerbs.includes(textLower) || textLower === 'co-founder') {
                ...
                if (subjTok && objTok) {
                    ...
                    const produced = tryCreateRelation(text, entities, spans, 'leads', subjSpan.start, subjSpan.end, objSpan.start, objSpan.end, 'DEP', tokens, tok.i);
                    addProducedRelations(produced, tok);
                }
                // Passive voice: "X was founded by Y" or "X founded by Y"
                // Look for "by" preposition attached to this verb
                const byPrep = tokens.find(t => t.dep === 'agent' && t.head === tok.i && t.text.toLowerCase() === 'by');
                if (byPrep) {
                    ...
                    if (founderTok) {
                        ...
                        if (orgTok) {
                            ...
                            const orgSpan = expandNP(orgTok, tokens);
                            const produced = tryCreateRelation(text, entities, spans, 'leads', founderSpan.start, founderSpan.end, orgSpan.start, orgSpan.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
            ...
            if (attendVerbs.includes(lemma) || attendVerbs.includes(textLower)) {
                const subjTok = resolveSubjectToken(tok, tokens);
                const atPrep = tokens.find(t => t.dep === 'prep' &&
                    t.head === tok.i &&
            ...
            if (graduateVerbs.includes(lemma) || graduateVerbs.includes(textLower)) {
                const subjTok = resolveSubjectToken(tok, tokens);
                const fromPrep = tokens.find(t => t.dep === 'prep' &&
                    t.head === tok.i &&
            ...
            if (advisorVerbs.includes(lemma) || advisorVerbs.includes(textLower)) {
                ...
                if (subjTok && objTok) {
                    const subjSpan = expandNP(subjTok, tokens);
                    const objSpan = expandNP(objTok, tokens);
                    const produced = tryCreateRelation(text, entities, spans, 'advised_by', objSpan.start, objSpan.end, subjSpan.start, subjSpan.end, 'DEP', tokens, tok.i);
                    addProducedRelations(produced, tok);
                }
            }
                ...
                if (toPrep) {
                    ...
                    if (adviseeTok) {
                        ...
                        if (advisorTok) {
                            ...
                            const produced = tryCreateRelation(text, entities, spans, 'advised_by', adviseeSpan.start, adviseeSpan.end, advisorSpan.start, advisorSpan.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
            ...
            if (investVerbs.includes(lemma) || investVerbs.includes(textLower)) {
                ...
                if (subjTok && inPrep) {
                    ...
                    if (objTok) {
                        const subjSpan = expandNP(subjTok, tokens);
                        const objSpan = expandNP(objTok, tokens);
                        const produced = tryCreateRelation(text, entities, spans, 'invested_in', subjSpan.start, subjSpan.end, objSpan.start, objSpan.end, 'DEP', tokens, tok.i);
                        addProducedRelations(produced, tok);
                    }
                }
            }
            ...
            if ((lemma === 'lead' || textLower === 'led') && textLower !== 'leader') {
                const roundObj = tokens.find(t => (t.dep === 'dobj' || t.dep === 'obj') &&
                    t.head === tok.i &&
            ...
            if (ruleVerbs.includes(lemma) || ruleVerbs.includes(textLower)) {
                ...
                if (subj && obj) {
                    const produced = tryCreateRelation(text, entities, spans, 'rules', subj.start, subj.end, obj.start, obj.end, 'DEP', tokens, tok.i);
                    addProducedRelations(produced, tok);
                }
                ...
                if (subj && overPrep) {
                    ...
                    if (place) {
                        const produced = tryCreateRelation(text, entities, spans, 'rules', subj.start, subj.end, place.start, place.end, 'DEP', tokens, tok.i);
                        addProducedRelations(produced, tok);
                    }
                }
            }
                    ...
                    if (personSubjects.length >= 2) {
                        for (let i = 0; i < personSubjects.length; i++) {
                            for (let j = i + 1; j < personSubjects.length; j++) {
                                ...
                                const forwardProduced = tryCreateRelation(text, entities, spans, 'friends_with', subjRef.span.start, subjRef.span.end, objRef.span.start, objRef.span.end, 'DEP', tokens, tok.i);
                                addProducedRelations(forwardProduced, tok);
                                // Create inverse relation (friends_with is symmetric)
                                const inverseProduced = tryCreateRelation(text, entities, spans, 'friends_with', objRef.span.start, objRef.span.end, subjRef.span.start, subjRef.span.end, 'DEP', tokens, tok.i);
            ...
            if (lemma === 'friend' || textLower === 'friends') {
                const uniqueTokensByStart = (items) => {
                    const map = new Map();
            ...
            if (livingVerbs.has(lemma) || livingVerbs.has(textLower)) {
                const relativeMarker = tokens.find(t => t.text.toLowerCase() === 'where' &&
                    t.start >= sent.start &&
                    t.start <= tok.start);
                let subjTok = resolveSubjectToken(tok, tokens);
                ...
                let placeRefFromRelative = null;
                if (!placeTok && relativeMarker) {
                    placeRefFromRelative = findNearestEntityBefore(relativeMarker.start, ['PLACE', 'ORG', 'HOUSE'], 400);
                }
            ...
            if (leadershipNouns.has(lemma) || leadershipNouns.has(textLower)) {
                ...
                if (orgTok && subj.length) {
                    const orgSpan = expandNP(orgTok, tokens);
                    const normalizedOrg = (0, entities_1.normalizeName)(text.slice(orgSpan.start, orgSpan.end));
                    ...
                    for (const subjTok of subj.flatMap(s => collectConjTokens(s, tokens))) {
                        ...
                        updateLastNamedSubject(resolvedSubj);
                        const subjSpan = expandNP(resolvedSubj, tokens);
                        const produced = tryCreateRelation(text, entities, spans, 'leads', subjSpan.start, subjSpan.end, targetSpan.start, targetSpan.end, 'DEP', tokens, tok.i);
                        addProducedRelations(produced, tok);
                    }
                }
            }
                ...
                if (apposBases.length) {
                    ...
                    if (parentEntityEntry) {
                        ...
                        for (const childTok of enumerated) {
                            const childSpan = expandNP(childTok, tokens);
                            const produced = tryCreateRelation(text, entities, spans, 'part_of', childSpan.start, childSpan.end, parentSpan.start, parentSpan.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
            ...
            if (lemma === 'sort') {
                ...
                if (intoPrep) {
                    ...
                    if (house && subjects.length) {
                        ...
                        const normalizedOrg = (0, entities_1.normalizeName)(text.slice(houseSpan.start, houseSpan.end));
                        const targetEntity = entities.find(e => e.canonical.toLowerCase() === normalizedOrg.toLowerCase());
                        ...
                        for (const subj of subjects.flatMap(s => collectConjTokens(s, tokens))) {
                            ...
                            updateLastNamedSubject(resolvedSubj);
                            const subjSpan = expandNP(resolvedSubj, tokens);
                            const produced = tryCreateRelation(text, entities, spans, 'member_of', subjSpan.start, subjSpan.end, targetSpan.start, targetSpan.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
            ...
            if (lemma === 'include') {
                ...
                if (baseObjects.length) {
                    const childTokens = [];
                    const seenChildStarts = new Set();
                    for (const objTok of baseObjects) {
                        for (const child of collectConjTokens(objTok, tokens)) {
                            if (!isNameToken(child))
                                continue;
                            if (seenChildStarts.has(child.start))
                                continue;
                    ...
                    if (childTokens.length) {
                        ...
                        if (!parentTokenCandidates.length) {
                            if (lastNamedSubject)
                                parentTokenCandidates.push(lastNamedSubject);
                            if (!parentTokenCandidates.length)
                                parentTokenCandidates.push(...recentPersons.slice(0, 2));
                        }
                        ...
                        for (const parentTok of parentTokenCandidates) {
                            ...
                            if (!entity || entity.type !== 'PERSON')
                                continue;
                            if (seenParentIds.has(entity.id))
                                continue;
                        ...
                        if (parentEntities.length && childEntities.length) {
                            for (const child of childEntities) {
                                for (const parent of parentEntities) {
                                    const produced = tryCreateRelation(text, entities, spans, 'child_of', child.span.start, child.span.end, parent.span.start, parent.span.end, 'DEP', tokens, tok.i);
                                    addProducedRelations(produced, tok);
                                }
                            }
                        }
            ...
            if (combatVerbs.includes(lemma) || combatVerbs.includes(textLower)) {
                ...
                if (subj && againstPrep) {
                    ...
                    if (enemy) {
                        const produced = tryCreateRelation(text, entities, spans, 'enemy_of', subj.start, subj.end, enemy.start, enemy.end, 'DEP', tokens, tok.i);
                        addProducedRelations(produced, tok);
                    }
                }
                ...
                if (['defeat', 'defeated', 'kill', 'killed', 'face', 'faced', 'faces', 'confront', 'confronted', 'confronts'].includes(lemma) ||
                    ...
                    const obj = tokens.find(t => (t.dep === 'dobj' || t.dep === 'obj') && t.head === tok.i);
                    if (subj && obj) {
                        const produced = tryCreateRelation(text, entities, spans, 'enemy_of', subj.start, subj.end, obj.start, obj.end, 'DEP', tokens, tok.i);
                        addProducedRelations(produced, tok);
                    }
                }
                // "fought in PLACE"
                if (['fight', 'fought', 'battle', 'battled'].includes(lemma)) {
                    ...
                    if (subj && inPrep) {
                        ...
                        if (place) {
                            const produced = tryCreateRelation(text, entities, spans, 'fought_in', subj.start, subj.end, place.start, place.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
            ...
            if (lemma === 'rival' || textLower.includes('rival')) {
                ...
                const mainVerb = tokens.find(t => (t.dep === 'ROOT' || t.pos === 'VERB' || t.pos === 'AUX') &&
                ...
                if (mainVerb || lemma === 'rival') {
                    ...
                    let subj = tokens.find(t => (t.dep === 'nsubj' && (t.head === verbIdx || t.head === tok.i)) ||
                    ...
                    const toPrep = tokens.find(t => t.dep === 'prep' &&
                    ...
                    if (subj && toPrep) {
                        ...
                        if (rival) {
                            ...
                            const produced = tryCreateRelation(text, entities, spans, 'enemy_of', subjSpan.start, subjSpan.end, rivalSpan.start, rivalSpan.end, 'DEP', tokens, tok.i);
                            addProducedRelations(produced, tok);
                        }
                    }
                }
        ...
        const handleChildrenEnumeration = () => {
            if (handledChildrenSentences.has(sent.start))
                return;
        ...
        const handleMembersEnumeration = () => {
            ...
            if (!items.length)
                return;
            const listStartAbs = sent.start + afterIndex;
            let containerRef = findNearestEntityBefore(sent.start + matchIndex, ['ORG', 'HOUSE', 'TRIBE'], 260);
            ...
            for (const surface of items) {
                ...
                if (!ref || ref.entity.type !== 'PERSON')
                    continue;
                if (memberRefs.some(r => r.entity.id === ref.entity.id))
                    continue;
                memberRefs.push({
                    entity: ref.entity,
                    span: ref.span ?? selectEntitySpan(ref.entity.id, spans, located)
            ...
            for (const member of memberRefs) {
                if (member.entity.id === container.entity.id)
                    continue;
                if (process.env.L3_ENUM_TRACE === '1') {
        ...
        const handleColonEnumeration = () => {
            ...
            if (handledColonPositions.has(colonAbs))
                return;
            ...
            for (const surface of items) {
                ...
                if (!ref)
                    continue;
                if (!['ORG', 'HOUSE', 'TRIBE'].includes(ref.entity.type))
                    continue;
                if (partRefs.some(r => r.entity.id === ref.entity.id))
                    continue;
                partRefs.push({
                    entity: ref.entity,
                    span: ref.span ?? selectEntitySpan(ref.entity.id, spans, located)
            ...
            for (const part of partRefs) {
                if (part.entity.id === container.entity.id)
                    continue;
                if (process.env.L3_ENUM_TRACE === '1') {
 ...
 */
function extractRegexRelations(text, entities, spans) {
    const relations = [];
    ...
    while ((match = sonOfPattern.exec(text))) {
        ...
        relations.push(...tryCreateRelation(text, entities, spans, 'child_of', childStart, childEnd, parentStart, parentEnd, 'REGEX'));
    }
    // Pattern 2: "X begat Y"
    ...
    while ((match = begatPattern.exec(text))) {
        ...
        const childEnd = childStart + childName.length;
        relations.push(...tryCreateRelation(text, entities, spans, 'parent_of', parentStart, parentEnd, childStart, childEnd, 'REGEX'));
    }
    ...
    while ((match = marriedPattern.exec(text))) {
        ...
        relations.push(...tryCreateRelation(text, entities, spans, 'married_to', name1Start, name1End, name2Start, name2End, 'REGEX'));
    }
    ...
    while ((match = travelPattern.exec(text))) {
        ...
        relations.push(...tryCreateRelation(text, entities, spans, 'traveled_to', personStart, personEnd, placeStart, placeEnd, 'REGEX'));
    }
    ...
    while ((match = studiesPattern.exec(text))) {
        ...
        const placeStart = match.index + match[0].lastIndexOf(place);
        const placeEnd = placeStart + place.length;
        relations.push(...tryCreateRelation(text, entities, spans, 'studies_at', personStart, personEnd, placeStart, placeEnd, 'REGEX'));
    }
    ...
    while ((match = attendedPattern.exec(text))) {
        ...
        const placeStart = match.index + match[0].lastIndexOf(place);
        const placeEnd = placeStart + place.length;
        relations.push(...tryCreateRelation(text, entities, spans, 'attended', personStart, personEnd, placeStart, placeEnd, 'REGEX'));
    }
    ...
    while ((match = teachesPattern.exec(text))) {
        ...
        const placeStart = match.index + match[0].lastIndexOf(place);
        const placeEnd = placeStart + place.length;
        relations.push(...tryCreateRelation(text, entities, spans, 'teaches_at', personStart, personEnd, placeStart, placeEnd, 'REGEX'));
    }
    ...
    while ((match = dweltPattern.exec(text))) {
        ...
        const placeStart = match.index + match[0].lastIndexOf(place);
        const placeEnd = placeStart + place.length;
        relations.push(...tryCreateRelation(text, entities, spans, 'lives_in', personStart, personEnd, placeStart, placeEnd, 'REGEX'));
    }
    ...
    while ((match = kingPattern.exec(text))) {
        ...
        const placeStart = match.index + match[0].lastIndexOf(place);
        const placeEnd = placeStart + place.length;
        relations.push(...tryCreateRelation(text, entities, spans, 'rules', personStart, personEnd, placeStart, placeEnd, 'REGEX'));
    }
    ...
    while ((match = friendsPattern.exec(text))) {
        ...
        relations.push(...tryCreateRelation(text, entities, spans, 'friends_with', person1Start, person1End, person2Start, person2End, 'REGEX'));
    }
    ...
    while ((match = combatPattern.exec(text))) {
        ...
        const person2Start = match.index + match[0].lastIndexOf(person2);
        const person2End = person2Start + person2.length;
        relations.push(...tryCreateRelation(text, entities, spans, 'enemy_of', person1Start, person1End, person2Start, person2End, 'REGEX'));
    }
    return relations;
}
/**
...
function dedupeRelations(relations) {
    const map = new Map();
    // Rank: dep=2, regex=1
    const rank = (r) => (r.extractor === 'dep' ? 2 : 1);
    for (const rel of relations) {
 ...
 */
async function extractRelations(text, result, docId) {
    const { entities, spans } = result;
    // Parse text to get dependency structure
    const parsed = await (0, entities_1.parseWithService)(text);
    const clauseAwareSentences = [];
    for (const sent of parsed.sentences) {
        ...
        for (const clause of clauses) {
            const clauseTokens = sent.tokens.filter(tok => tok.start >= clause.start && tok.end <= clause.end);
