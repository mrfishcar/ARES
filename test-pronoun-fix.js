...
const { extractEntitiesAndRelations } = require('./dist/app/engine/extract/orchestrator.js');
...
async function testPronounFix() {
  console.log('🧪 Testing Pronoun Fix (Claude Online Refactor)');
  ...
  if (frederick && saul && frederick.id !== saul.id) {
    console.log('  ✅ Frederick and Saul are SEPARATE entities');
    console.log('     (No pronoun-based merge - bug FIXED!)');
  } else if (frederick && saul) {
    console.log('  ❌ Frederick and Saul were MERGED into one entity');
    console.log('     (Pronoun bug still exists)');
  }
