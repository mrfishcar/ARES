/**
 * Test extraction on Sherlock Holmes stories
 * "A Scandal in Bohemia" + "The Red-Headed League"
 * ~17,600 words from Project Gutenberg (public domain)
 */
...
async function testSherlock() {
  console.log('SHERLOCK HOLMES STORY EXTRACTION TEST');
  console.log('='.repeat(80));

  const text = fs.readFileSync('/tmp/sherlock-scandal.txt', 'utf-8');
  ...
  const { entities, relations } = await extractFromSegments('sherlock', text);
